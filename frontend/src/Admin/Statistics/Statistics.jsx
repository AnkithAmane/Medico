...
export default function Normal_Stats() {
  ...
  // 🟢 DYNAMIC LOGIC: Automatically derive valid current year, month, and week configuration strings
  const currentYearInt = new Date().getFullYear(); // e.g. 2026
  const currentMonthStr = new Date().toISOString().slice(0, 7); // e.g. "2026-05"

  // Calculate current fallback week notation format dynamically (e.g., "2026-W20")
  ...
  // Filter States initialized purely with structural dynamic variables
  ...
  const syncClinicalIntelligence = async () => {
    try {
      ...
      const [apptRes, patRes, docRes] = await Promise.all([
        ...
        axios.get("http://localhost:5000/api/doctors/list", { headers }), // 🟢 FIXED: Remapped from /all to /list
      ]);
        ...
        datasets: [
          {
            ...
            data: [
              dbData.appointments.length > 0
                ? Math.round(dbData.patients.length * 0.55)
                : 0,
              dbData.appointments.length > 0
                ? Math.round(dbData.patients.length * 0.28)
                : 0,
              dbData.appointments.length > 0
                ? Math.round(dbData.patients.length * 0.12)
                : 0,
              dbData.appointments.length > 0
                ? Math.round(dbData.patients.length * 0.05)
                : 0,
            ],
  ...
  }, [dbData.patients, dbData.appointments, patAcqYear, patDemoFilter]);

  /* --- 4. DOCTOR PERFORMANCE LOGIC --- */
  const doctorCharts = useMemo(() => {
    // 🟢 FIXED: Extracted distinct clinical categories dynamically straight from current doctor arrays tracking logs
    ...
    // 🟢 FIXED: Unified lower-case match tracking matrix
    ...
    return {
      ...
      bestPerformer: {
        name: topDocName,
        // 🟢 FIXED: Safe fallback sequence uses native SVG string profiles when a remote asset fails to respond
        img: topDocObj?.photo
  ...
  const appointmentCharts = useMemo(() => {
    ...
    // 🟢 FIXED: Compute distinct active traffic labels matrix directly from selection logs
    const trafficLabels =
        ...
        datasets: [
          {
            data: [
              ...
              dbData.appointments.filter((a) => a.status === "Cancelled")
                .length,
              dbData.appointments.filter((a) => a.status === "Upcoming").length,
              12,
            ],
  ...
  if (loading)
    return (
      <div className="admin_dash_load">
        <Loader2 className="spin" /> Synchronizing Intelligence Core...
      </div>
    );
    ...
    <div className="admin_stat_m_revenue_wrapper doc_home_view_fade_in">
      ...
      <header className="admin_stat_m_revenue_header">
        <div className="admin_stat_m_header_text">
          ...
          <p>Global Intelligence • {lastSynced}</p>
        </div>
                ...
                <div className="admin_stat_m_best_doctor_profile">
                  ...
                  <div className="admin_stat_m_doc_info_row">
                    <div className="admin_stat_m_info_box">
                      <span>Specialty</span>
                      format<strong>{doctorCharts.bestPerformer.dept}</strong>
                    </div>
                    <div className="admin_stat_m_info_box">
