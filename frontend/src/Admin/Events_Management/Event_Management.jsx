...
export default function Event_Management() {
  ...
  const initializeWorkspace = async () => {
    ...
    try {
      ...
      const [eventRes, doctorRes] = await Promise.all([
  ...
  // Handles real-time property target updates cleanly
  const handleFormChange = (e) => {
    setFormFields({ ...formFields, [e.target.name]: e.target.value });
  };

  const handleToggleDepartmentSelection = (dept) => {
    setFormFields((prev) => {
      const exists = prev.department.includes(dept);
      const updatedDepts = exists
        ? prev.department.filter((d) => d !== dept)
        : [...prev.department, dept];

      return {
        ...prev,
        department: updatedDepts.length > 0 ? updatedDepts : prev.department,
        doctors: [], // Flushes panel roster on layout shifts to avoid data overlaps
      };
    });
  };
  ...
  const handleOpenEditModal = (e, event) => {
    e.stopPropagation(); // Stops the full background row detail viewing selector from triggering simultaneously
  ...
  const handleCreateSubmit = async (e) => {
    ...
    try {
      ...
      // Clear all setup fields contexts
      setFormFields({
        title: "",
        department: ["General Medicine"],
        date: "",
  ...
  if (loading)
    return (
      <div className="admin_dash_load">
        ...
        <p>Synchronizing Multi-Department Registries & Roster Grids...</p>
      </div>
    );
    ...
    <div className="event_elite_root doc_home_view_fade_in">
      ...
      <div className="dept_mgmt_control_bar">
        ...
        <div style={{ display: "flex", gap: "10px" }}>
          ...
          >
            ...
            {AVAILABLE_DEPARTMENTS.map((d, idx) => (
      ...
      {eventState.showEventForm && (
        <div className="admin_appt_m_modal">
          <div
            className="admin_appt_m_modal_box dept_modal_wide"
          ...
          >
            <div className="admin_appt_m_modal_head">
              ...
              <button onClick={() => setState({ showEventForm: false })}>
                <X size={20} />
              </button>
            </div>
            <form className="admin_appt_m_form" onSubmit={handleCreateSubmit}>
              ...
              {/* MULTI-DEPARTMENT SELECTION CHIPS BAR CONTAINER */}
              <div
              ...
              >
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "8px" }}>
                  {AVAILABLE_DEPARTMENTS.map((dept, i) => {
                    const isSelected = formFields.department.includes(dept);
                    return (
                      <button
                        type="button"
                        key={i}
                        onClick={() => handleToggleDepartmentSelection(dept)}
                        style={{
                          padding: "6px 14px",
                          borderRadius: "20px",
                          border: "1px solid",
              ...
              {/* DUAL-METHOD SPECIALIST STAFF ASSIGNMENT CONTROL GRID */}
              <div
                className="doctor_assignment_wrapper"
                style={{
                  margin: "15px 0",
                  background: "#f8fafc",
                  padding: "15px",
              ...
              >
                ...
                {doctorSelectionMethod === "department" ? (
                  <div>
                    ...
                    >
                      Sync All ({filteredDoctorsByDepts.length}) Medical Experts
