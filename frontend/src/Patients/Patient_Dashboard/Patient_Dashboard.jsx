...
import PendingFeedbackPanel from "../Feedback/Feeback_Panel"; // Adjust path if needed
import {
  Activity,
  Scale,
  Ruler,
...
export default function Patient_Dashboard() {
  ...
  /* --- 2. Live Appointment Fetching --- */
  useEffect(() => {
    const fetchAppointments = async () => {
      try {
        const token = localStorage.getItem("token");
        // Use user._id as the primary key for the backend request
        const identifier = user._id || user.name;
  ...
  const handleItemClick = (apptId) => {
    // Navigates to bookings and passes state to auto-select the card
    navigate("/patient/patient_bookings", { state: { autoSelectId: apptId } });
  };

  /* --- 4. Memoized Logic for Lists --- */
      ...
      <div className="pat_dash_main_content">
        ...
        <div className="pat_dash_row_1">
          <div className="pat_dash_welcome">
            <h1 className="pat_dash_h1">
              {getGreeting()},{" "}
              <span className="pat_dash_span">{user.name.split(" ")[0]}!</span>
            </h1>
        ...
        <div className="pat_dash_row_3_vitals">
          <div className="pat_dash_vitals_section_header">
            <h3 className="pat_dash_h3">Live Health Metrics</h3>
          </div>
        ...
        <div className="pat_dash_sidebar_widget pat_dash_widget_calendar">
          ...
          <div className="pat_dash_calendar_grid_ui">
            ...
            {[...Array(31)].map((_, i) => (
              <span
                key={i}
                className={`pat_dash_calendar_date_cell ${i + 1 === dateTime.getDate() ? "pat_dash_calendar_today_active" : ""}`}
              >
                {i + 1}
              </span>
            ))}
